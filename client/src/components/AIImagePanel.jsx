...
const panelStyles = `
.ai-image-panel { display:flex; flex-direction:column; height:100%; overflow:hidden; background:#fff; }

/* Header */
.aip-header { padding:1rem 1.1rem 0.75rem; border-bottom:1px solid #e2e8f0; flex-shrink:0; }
.aip-title { font-size:0.72rem; font-weight:700; text-transform:uppercase; letter-spacing:.06em; color:#374151; margin-bottom:0.6rem; display:flex; align-items:center; gap:6px; }
.aip-title svg { color:#8b5cf6; }

/* Search bar */
.aip-search-row { display:flex; gap:6px; }
.aip-search-input {
  flex:1; padding:6px 10px; border:1px solid #d1d5db; border-radius:6px;
  font-size:0.8rem; outline:none; background:#f9fafb;
  transition:border-color .15s;
}
.aip-search-input:focus { border-color:#8b5cf6; }
.aip-search-btn {
  padding:6px 9px; background:#8b5cf6; color:#fff; border:none;
  border-radius:6px; cursor:pointer; display:flex; align-items:center;
  transition:background .15s;
}
.aip-search-btn:hover { background:#7c3aed; }
.aip-refresh-btn {
  padding:6px 9px; background:#f1f5f9; color:#6b7280; border:1px solid #e2e8f0;
  border-radius:6px; cursor:pointer; display:flex; align-items:center;
  transition:all .15s;
}
...
/* Keyword badges */
.aip-keywords { display:flex; flex-wrap:wrap; gap:4px; padding:0.65rem 1.1rem; border-bottom:1px solid #f1f5f9; flex-shrink:0; }
.aip-kw-badge {
...
.aip-body { flex:1; overflow-y:auto; padding:0.85rem 1.1rem; display:flex; flex-direction:column; gap:1.2rem; }

/* Image slot */
.aip-slot { display:flex; flex-direction:column; gap:0.5rem; }
.aip-slot-label { font-size:0.67rem; font-weight:600; color:#6b7280; text-transform:uppercase; letter-spacing:.05em; }
...
.aip-primary-img-wrap:hover { border-color:#8b5cf6; box-shadow:0 0 0 3px rgba(139,92,246,.15); }
.aip-primary-img-wrap.selected { border-color:#10b981; box-shadow:0 0 0 3px rgba(16,185,129,.15); }
...
.aip-overlay-btn {
  padding:5px 10px; border-radius:5px; border:none; font-size:0.73rem; font-weight:600;
  cursor:pointer; display:flex; align-items:center; gap:4px;
}
.aip-overlay-btn.add { background:#8b5cf6; color:#fff; }
.aip-overlay-btn.add:hover { background:#7c3aed; }
.aip-overlay-btn.ext { background:rgba(255,255,255,.22); color:#fff; }
.aip-added-badge {
  position:absolute; top:6px; right:6px; background:#10b981; color:#fff;
  border-radius:4px; font-size:0.64rem; font-weight:700; padding:2px 6px;
  display:flex; align-items:center; gap:3px;
}
.aip-img-source {
  position:absolute; bottom:6px; left:6px; background:rgba(0,0,0,.55); color:#fff;
  font-size:0.6rem; border-radius:3px; padding:1px 5px;
}
...
/* Alternatives strip */
.aip-alts-toggle {
  background:none; border:none; font-size:0.72rem; color:#6b7280;
  cursor:pointer; display:flex; align-items:center; gap:4px; padding:0;
}
.aip-alts-toggle:hover { color:#374151; }
.aip-alts-strip { display:flex; gap:5px; overflow-x:auto; padding-bottom:4px; }
.aip-alt-thumb {
  flex-shrink:0; width:70px; height:46px; border-radius:5px; overflow:hidden;
  border:2px solid transparent; cursor:pointer; transition:border-color .12s;
  position:relative;
}
.aip-alt-thumb:hover { border-color:#8b5cf6; }
.aip-alt-thumb.selected { border-color:#10b981; }
.aip-alt-thumb img { width:100%; height:100%; object-fit:cover; display:block; }
.aip-alt-thumb-skeleton {
  width:70px; height:46px; border-radius:5px; flex-shrink:0;
  background:linear-gradient(90deg,#f0f0f0 25%,#e0e0e0 50%,#f0f0f0 75%);
  background-size:200% 100%; animation:aip-shimmer 1.4s infinite;
}

/* Empty state */
.aip-empty { text-align:center; padding:2rem 1rem; color:#9ca3af; font-size:0.82rem; }
.aip-empty svg { margin-bottom:8px; opacity:.4; }

/* Tip banner */
.aip-tip {
  margin:0 1.1rem 0.75rem; padding:0.65rem 0.75rem;
  background:linear-gradient(90deg,#faf5ff,#eff6ff); border:1px solid #e9d5ff;
  border-radius:7px; font-size:0.72rem; color:#5b21b6; line-height:1.5;
}
`;
      ...
      <div className="ai-image-panel">
        ...
        <div className="aip-header">
          <div className="aip-title">
            <Sparkles size={13} /> AI Image Search
          </div>
          <div className="aip-search-row">
            ...
            <button className="aip-refresh-btn" onClick={handleRefresh} title="Refresh">
              <RefreshCw size={14} className={loading ? 'aip-spin' : ''} />
            </button>
          </div>
        </div>
            ...
            <div className="aip-slot">
              ...
              {loading ? (
                ...
                >
                  ...
                  {addedSuccess && (
                    <div className="aip-added-badge"><Check size={9} /> Added!</div>
                  )}
              ...
              {/* Attribution */}
              {currentImage?.author && (
                <div style={{ fontSize: '0.64rem', color: '#9ca3af', marginTop: 2 }}>
                  Photo by{' '}
                  <a href={currentImage.authorUrl} target="_blank" rel="noreferrer" style={{ color: '#6b7280' }}>
                    {currentImage.author}
                  </a>{' '}
                  · {currentImage.source}
                </div>
