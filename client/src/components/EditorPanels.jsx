...
const useKaTeX = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Keep loaded assets on head/body to avoid reloading
    };
  }, []);
...
export default function EditorPanels({
  ...
  activeElement, // selected element if any
  ...
  return (
    <div 
      ...
      style={{
        position: 'absolute',
        top: '64px', // directly below the top toolbar
        left: 0,
        right: 0,
        height: '380px',
        zIndex: 500,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontWeight: 600, fontSize: '0.95rem', color: '#1e293b', textTransform: 'capitalize' }}>
            {activePanel === 'autolayout' ? 'Auto Layout' : activePanel} Control Panel
          </span>
        </div>
        <button 
          onClick={onClose}
...
function TextPanel({ onInsert, activeElement, onUpdateElementStyle }) {
  const [tab, setTab] = useState('text'); // 'text' or 'customize'
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%' }}>
      {/* Side Tabs */}
      <div style={{ width: '150px', borderRight: '1px solid #e2e8f0', background: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
        <button 
          onClick={() => setTab('text')}
          style={{
            padding: '12px 16px',
      ...
      <div style={{ flex: 1, padding: '20px 24px', overflowY: 'auto' }}>
        {tab === 'text' ? (
          <div>
            <h4 style={{ margin: '0 0 16px 0', color: '#1e293b', fontSize: '0.9rem' }}>Choose a preset to insert</h4>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
              {presets.map((p, idx) => (
                ...
                >
                  <span style={{ fontSize: '0.72rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>{p.label}</span>
                  <div style={{ 
                    fontSize: p.size === '44px' ? '1.15rem' : p.size === '32px' ? '0.95rem' : '0.82rem', 
                    fontWeight: p.weight === 'bold' ? 'bold' : 'normal',
                    fontStyle: p.weight === 'italic' ? 'italic' : 'normal',
                    textAlign: p.align,
                    color: '#0f172a',
          ...
          <div style={{ maxWidth: '400px' }}>
            <h4 style={{ margin: '0 0 16px 0', color: '#1e293b', fontSize: '0.9rem' }}>Customize Active Text Settings</h4>
            {activeElement ? (
...
function TablePanel({ onInsert }) {
  ...
  return (
    <div style={{ display: 'flex', width: '100%', padding: '20px 24px', boxSizing: 'border-box' }}>
      {/* 20x20 Grid Selector */}
      <div style={{ borderRight: '1px solid #e2e8f0', paddingRight: '28px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '2px', background: '#f1f5f9', padding: '6px', borderRadius: '6px' }}>
          {Array(10).fill(null).map((_, rIdx) => (
            Array(12).fill(null).map((_, cIdx) => {
              const r = rIdx + 1;
              const c = cIdx + 1;
...
function ChartPanel({ onInsert }) {
  ...
  const chartCategories = ['All', 'Line', 'Bar', 'Pie', 'Scatter', 'Other'];
  ...
  const charts = [
    ...
    { name: 'Doughnut Breakdown', type: 'doughnut', category: 'Pie' },
    { name: 'Scatter Plot Matrix', type: 'scatter', category: 'Scatter' },
    { name: 'Gradient Area Chart', type: 'area', category: 'Line' },
    { name: 'Funnel Stage Metrics', type: 'funnel', category: 'Other' },
    { name: 'Radar Target Map', type: 'radar', category: 'Other' }
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', padding: '16px 24px', boxSizing: 'border-box' }}>
      ...
      {/* Grid */}
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', overflowY: 'auto' }}>
        {filteredCharts.map((c, idx) => (
          <div
            ...
            className="hover-card-border"
          >
            <div style={{ width: '100%', height: '55px', background: '#f8fafc', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a855f7' }}>
              <BarChart3 size={24} />
            </div>
            <span style={{ fontSize: '0.74rem', color: '#1e293b', fontWeight: 600, marginTop: '8px', textAlign: 'center' }}>{c.name}</span>
          </div>
        ))}
      </div>
    </div>
...
function ImagePanel({ onInsert }) {
  const [tab, setTab] = useState('pexels'); // 'pexels' | 'upload' | 'themes'
  const [search, setSearch] = useState('');

  const themes = ['Business', 'Technology', 'Education', 'Nature', 'Finance', 'Creative'];

  const sampleImages = [
    { url: 'https://images.unsplash.com/photo-1557804506-669a67965ba0?w=500&auto=format&fit=crop&q=60', desc: 'Team Meeting' },
    { url: 'https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?w=500&auto=format&fit=crop&q=60', desc: 'Cyber Tech Code' },
    { url: 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=500&auto=format&fit=crop&q=60', desc: 'Books Study' },
    { url: 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=500&auto=format&fit=crop&q=60', desc: 'Green Forest' }
  ];
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%' }}>
      ...
      <div style={{ width: '130px', borderRight: '1px solid #e2e8f0', background: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
        {[
          { id: 'pexels', label: 'Pexels / Pixabay' },
      ...
      <div style={{ flex: 1, padding: '16px 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', overflowY: 'auto', flex: 1 }}>
              {sampleImages.map((img, idx) => (
                <div 
                  ...
                  style={{ borderRadius: '6px', overflow: 'hidden', height: '100px', cursor: 'pointer', border: '1px solid #f1f5f9', position: 'relative' }}
                >
        ...
        {tab === 'themes' && (
          <div>
            <h4 style={{ margin: '0 0 12px 0', fontSize: '0.82rem', color: '#1e293b' }}>Image Themes</h4>
...
function ShapePanel({ onInsert }) {
  ...
  const categories = ['All', 'Basic Shapes', 'Arrows', 'Flowchart', 'Stars'];

  const shapes = [
    { name: 'Rectangle', type: 'rectangle', cat: 'Basic Shapes', svg: <rect x="5" y="5" width="40" height="40" fill="currentColor" rx="4" /> },
    { name: 'Circle', type: 'circle', cat: 'Basic Shapes', svg: <circle cx="25" cy="25" r="20" fill="currentColor" /> },
    { name: 'Triangle', type: 'triangle', cat: 'Basic Shapes', svg: <polygon points="25,5 45,45 5,45" fill="currentColor" /> },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
          {filteredShapes.map((shape, idx) => (
            <div
              ...
              className="hover-card-border"
            >
              <svg width="40" height="40" viewBox="0 0 50 50" style={{ color: '#7c3aed' }}>
                {shape.svg}
              </svg>
              <span style={{ fontSize: '0.64rem', color: '#475569', marginTop: '6px', textAlign: 'center', fontWeight: 500 }}>{shape.name}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div style={{ display: 'flex', width: '100%', height: '100%' }}>
      ...
      <div style={{ flex: 1, padding: '16px 20px', display: 'flex', gap: '20px', overflowY: 'auto' }}>
        ...
        <div style={{ flex: 1, borderLeft: '1px solid #e2e8f0', paddingLeft: '16px', maxHeight: '280px', overflowY: 'auto' }}>
          {tab === 'formulas' ? (
            <div>
              <span style={{ fontSize: '0.74rem', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '8px' }}>Click to apply code</span>
            ...
            <div>
              <span style={{ fontSize: '0.74rem', fontWeight: 600, color: '#64748b', display: 'block', marginBottom: '8px' }}>Inject math symbols</span>
...
function AutoLayoutPanel({ onApplyAutoLayout, selectedElements }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', padding: '20px 24px', boxSizing: 'border-box' }}>
      <h4 style={{ margin: '0 0 4px 0', fontSize: '0.88rem', color: '#1e293b' }}>Auto Layout & Position Alignments</h4>
      <p style={{ margin: '0 0 16px 0', fontSize: '0.72rem', color: '#64748b' }}>
        Select one or more elements on the canvas, then click any alignment to organize them immediately.
      </p>
...
function AssetPanel({ onInsert }) {
  ...
  const sampleAssets = {
    ...
    illustrations: [
      ...
      { label: 'Data Analysis Chart Illustration', code: '📊' },
      { label: 'Secure Security Illustration', code: '🔒' }
    ]
  };
...
function MindMapPanel({ onInsert }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', padding: '20px 24px', boxSizing: 'border-box' }}>
      <h4 style={{ margin: '0 0 4px 0', fontSize: '0.88rem', color: '#1e293b' }}>Insert Node Mind Map</h4>
      <p style={{ margin: '0 0 16px 0', fontSize: '0.72rem', color: '#64748b' }}>
        Select a diagram type below to add a dynamic, fully-editable mind map chart node to your active slide.
      </p>
